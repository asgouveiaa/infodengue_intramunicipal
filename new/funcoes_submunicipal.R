...
generate_maps <- function(data, weeks_2_plotdistrito_nome, lim_range = NULL,
  ...
  if (is.null(lim_range)) {
    filtered_data <- data %>% filter(sem_not %in% weeks_2_plot)
  ...
  plots_list <- map(weeks_2_plot, ~ {
    week_data <- data %>% filter(sem_not == .x)
    
    # Calcular porcentagem de não alocados (se aplicável)
    percent_na <- if(exists("jv_intr_dados$df_mun_ok") && exists("jv_intr_dados$df_mun")) {
      100 - (jv_intr_dados$df_mun_ok %>% filter(sem_not == .x) %>% nrow() / 
               jv_intr_dados$df_mun %>% filter(sem_not == .x) %>% nrow() * 100)
    } else 0
...
# Função para calcular semanas consecutivas com nível = 1 
# calcular_semanas_consecutivas <- function(data, semana_atual, bairro, janela) {
...
#   pos_atual <- which(dados_bairro$sem_not == semana_atual)
#   
#   if (length(pos_atual) == 0) return(0)
#   
#   # Definir janela de análise (até 10 semanas anteriores)
#   inicio_janela <- max(1, pos_atual - janela + 1)  select(nm_bairro_ref, casos,  inc, Rt,
...
# Função para contar semanas consecutivas anteriores com nível = 1
# calcular_semanas_consecutivas <- function(data, semana_atual, bairro, janela = 10) {
#   # Validação de entrada
#   if (missing(data) || missing(semana_atual) || missing(bairro)) {
#     stop("Parâmetros obrigatórios: data, semana_atual e bairro devem ser fornecidos")
#   }
#   
#   if (!is.numeric(janela) || janela <= 0) {
#     stop("janela deve ser um número positivo")
#   }
#   
...
#   
#   # Retorno antecipado se não há dados para este bairro
...
#   }
#   
#   # Se não há semanas anteriores, retorna 0
#   if (pos_atual == 1) {
#     return(0)
#   }
#   
#   # Definir janela de análise (semanas anteriores à atual)
#   # pos_atual - 1 porque queremos apenas as semanas ANTERIORES
#   ultima_semana_anterior <- pos_atual - 1
#   inicio_janela <- max(1, ultima_semana_anterior - janela + 1)
#   
#   # Extrair níveis das semanas anteriores
#   niveis_anteriores <- dados_bairro$nivel[inicio_janela:ultima_semana_anterior]
#   
#   if (length(niveis_anteriores) == 0) {
#     return(0)
#   }
#   
#   # Contar semanas consecutivas com nível = 1 da mais recente para trás
#   # (começando da semana imediatamente anterior à atual)
#   semanas_consecutivas <- 0
#   for (i in length(niveis_anteriores):1) {
#     if (!is.na(niveis_anteriores[i]) && niveis_anteriores[i] == 1) {
#       semanas_consecutivas <- semanas_consecutivas + 1
#     } else {
#       break
#     }
